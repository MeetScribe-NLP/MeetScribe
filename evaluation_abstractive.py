...
for name, fname in summary_files.items():
    print(f"\nEvaluating {name}...")
    sum_df = pd.read_csv(os.path.join(data_folder, fname), compression="gzip")
    ...
    out_path   = os.path.join(data_folder, f"{name}_metrics.csv")
    metrics_df.to_csv(out_path, index=False)
    metrics_map[name] = metrics_df
    print(f"Saved metrics to {out_path}")

# 7. Compute overall scores & ranking
overall_scores = {}
for name, dfm in metrics_map.items():
    # average of coherence, coverage, fluency
    mean_scores = dfm[["coherence","coverage","fluency"]].mean()
    overall_scores[name] = mean_scores.mean()

# Separate transcript-only vs metadata
trans_keys = [k for k in overall_scores if "transcript" in k]
meta_keys  = [k for k in overall_scores if "metadata" in k]

print("\n=== Transcript-only Models Ranking ===")
for k in sorted(trans_keys, key=lambda x: overall_scores[x], reverse=True):
    print(f"{k}: {overall_scores[k]:.3f}")
print(f"Best transcript-only: {max(trans_keys, key=lambda x: overall_scores[x])}")

print("\n=== Metadata-augmented Models Ranking ===")
for k in sorted(meta_keys, key=lambda x: overall_scores[x], reverse=True):
    print(f"{k}: {overall_scores[k]:.3f}")
print(f"Best metadata-augmented: {max(meta_keys, key=lambda x: overall_scores[x])}")
